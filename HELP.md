...
- **Paramètres (IA)** : icône **engrenage** à gauche du titre « Resume YouTube » dans la barre latérale ; le bouton **Logs** est à droite sur la même ligne (plus d’affichage du chemin vers `chaines.json` dans l’UI). Section **Fournisseur IA** : onglets **LM Studio** (adresse du serveur **fixe dans l’application** : `http://192.168.1.71:1234`, sans champ dans l’interface ; choix du modèle et liste via `GET …/v1/models`, puis `GET …/api/v1/models` si la route OpenAI répond **404** ou renvoie une liste **sans identifiant**), **OpenRouter** (champ **clé API** — à saisir par l’utilisateur, rien n’est prérempli dans le code ; la valeur est mémorisée dans le stockage local ; liste des modèles **gratuits** filtrés sur `id` contenant `:free` via l’API publique `GET https://openrouter.ai/api/v1/models`) et **Local** (GGUF MARTHA sous `donnees/modeles_locaux/`, inférence llama.cpp dans le processus Tauri). Le fournisseur actif (onglet sélectionné) et les champs sont mémorisés dans `localStorage`. Les résumés utilisent le même schéma `lm` (camelCase) : `fournisseur` (`lm_studio`, `openrouter` ou `local`), `baseUrl` (LM Studio, côté app fixe), `cleLmStudio` (non utilisé côté app), `cleOpenrouter`, `modele`, `temperature`, et `nCtx` pour le mode **Local**.
...
3. IA (LM Studio ou OpenRouter) :
   - **LM Studio** : `POST` vers `{baseUrl}/v1/chat/completions` (`baseUrl` par défaut côté app : `http://192.168.1.71:1234`).
...
| `lister_modeles_lm_studio` | Liste des modèles LM Studio : `GET /v1/models` (OpenAI, `data[].id` ou `data[].model`) ; si **404** ou si la réponse **200** ne contient aucun identifiant, repli sur `GET /api/v1/models` (REST, `models[].key`) ; base par défaut côté app : `http://192.168.1.71:1234`. |
...
Le dépôt Git local est prêt (branche `main`). Les gros artefacts (`donnees/modeles_locaux/`, `bin/**`, binaires et archives sous `dist/`) sont ignorés par `.gitignore` pour rester sous la limite GitHub.
